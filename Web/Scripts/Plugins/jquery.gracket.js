    ...
    $.fn.gracket = function(method) {
        ...
        var methods = {
            init: function(options) {
                ...
                for (var r = 0; r < round_count; r++) {
                    ...
                    for (var g = 0; g < game_count; g++) {
                        ...
                        for (var t = 0; t < team_count; t++) {
                            ...
                            if (data[r][g].length == 2) {
                                if (t == 1 && data[r][g][0].seed != data[r][g][1].seed) {
                                    game_html.find('.g_seed').first().css('color', data[r][g][0].seed > data[r][g][1].seed ? 'green' : 'red');
                                    game_html.find('.g_seed').last().css('color', data[r][g][1].seed > data[r][g][0].seed ? 'green' : 'red');
                                }
                            }
                        };
                    };
                ...
                $.each(container.find('.g_team'), function () {
                    var classes = $(this)[0].className.split(' ');
                    if (classes[1] == -1) {
                        return;
                    }
                    var allPositions = container.find('.' + classes.join('.'));
                    $(this).hover(function() {
                        allPositions.toggleClass('gracket-hightlight');
                    });
                });
            }
};
